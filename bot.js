...
var TICKER='$FWC26';
var CA='0xC4c6EaC95c5619F45714b74a4c4503f00af1FA26';
var TWITTER='https://x.com/FWC26Bsc';
var TG='https://t.me/FWC2026BSC';
var WEBSITE='';
var IS_CTO=false;
...
loadState();
var _IMG1=path.join(__dirname,'fwc26.jpg');
...
var NOT_LIVE=['$FWC26 hasn\u2019t launched yet. CA coming soon.','Not yet. Stay ready.','CA drops soon. Hold tight.'];
var CTO_REPLIES=['$FWC26 is a CTO. Original dev gone. Community owns and runs this completely. No dev to rug.','CTO project. Dev walked away. Community stepped up and owns $FWC26 now. That is the strength.','No dev here. $FWC26 is 100% community-owned. Original dev left. Community drives this forward.'];
function sysPrompt(){
  return 'You are the community bot for $FWC26, a BNB Smart Chain (BSC) meme token.\nToken: $FWC26 | Chain: BNB Smart Chain (BSC)\nSupply: N/A\nTax: 5% buy / 5% sell\nContract: NOT RENOUNCED | LP: LOCKED\nDEV: Active, building, present. Never imply dev left.'+(TWITTER?'\nTwitter: '+TWITTER:'')+'\nNarrative: '+"The Official Mascot Token of FIFA WorldCup 2026 on BSC\nBuilt with PREDICT & WIN platform\nA platform where you PREDICT and Win in every\nMatch with no betting or staking,\nAs 4% from buy and sell tax goes into the VAULT to reward you.\nFIFA WORLD CUP 2026 BSC OG.\n\n\nTotal tax is 5% for buy and 5% for sell\n\n3/3 out if it go to marketing and 2/2 go to vault, it makes it 6% for marketing and 4% for prediction vault"+'\nPersonality: Confident, sharp, crypto-native. Talk like a seasoned degen who believes in the project. Direct and bold.\nRULES: 2-4 lines max. Natural and professional. Never share TG group link. Never repeat reply. If hype/casual/no question: reply IGNORE exactly.';
}
...
var SIL_ANG=['2-3 lines. Why hold $FWC26 right now.','2-3 lines. $FWC26 fundamentals: renounced, LP locked.','2-3 lines. Being early to $FWC26.','2-3 lines. $FWC26 community is building.','2-3 lines. The move in $FWC26 is still early.'];
var silIdx=0;
...
async function doShoutout(){
  ...
  try{
    ...
    var ppt='Write 1-2 warm genuine lines appreciating these $FWC26 admins for keeping the community alive: '+names.join(', ')+'. Be specific, sound human, tag them by name.';
...
bot.command('ca',async function(ctx){if(!caUnlocked)return ctx.reply(NOT_LIVE[Math.floor(Math.random()*NOT_LIVE.length)]);await sendWithTracker(caMsg,ctx.chat.id,'$FWC26 Contract Address',{});return ctx.reply('<code>'+CA+'</code>',{parse_mode:'HTML'});});
bot.command('x',async function(ctx){return sendWithTracker(xMsg,ctx.chat.id,'Follow $FWC26 on X',{reply_markup:{inline_keyboard:[[{text:'Follow on X',url:TWITTER}]]}});});
bot.command('twitter',async function(ctx){return sendWithTracker(xMsg,ctx.chat.id,'Follow $FWC26 on X',{reply_markup:{inline_keyboard:[[{text:'Follow on X',url:TWITTER}]]}});});
bot.command('socials',function(ctx){return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xC4c6EaC95c5619F45714b74a4c4503f00af1FA26\'>Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xC4c6EaC95c5619F45714b74a4c4503f00af1FA26\'>PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':'')+(WEBSITE?' | <a href=\''+WEBSITE+'\'>Website</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});});
bot.command('links',function(ctx){return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xC4c6EaC95c5619F45714b74a4c4503f00af1FA26\'>Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xC4c6EaC95c5619F45714b74a4c4503f00af1FA26\'>PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':'')+(WEBSITE?' | <a href=\''+WEBSITE+'\'>Website</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});});
bot.command('info',function(ctx){return ctx.reply('<b>$FWC26</b> \u2014 BNB Smart Chain (BSC)\n\nSupply: N/A\nTax: 5% buy / 5% sell\nContract: NOT RENOUNCED\nLP: LOCKED'+(TWITTER?'\nTwitter: '+TWITTER:''),{parse_mode:'HTML',disable_web_page_preview:true});});
bot.command('shill',async function(ctx){
  var shillMsgs=[
    'Have you heard about $FWC26?\n\n$FWC26 \u2014 community-owned on BSC.\nRenounced. LP LOCKED. No dev games.\nThis is the quiet move. Load up.',
    'Looking for a BSC token built by real people?\n\n$FWC26 \u2014 fully community-owned.\nRenounced contract. LP LOCKED. Real narrative.\nGet in early \u261d',
    'The move others will regret missing.\n\n$FWC26 on BSC \u2014 community takeover.\nRenounced. LP LOCKED. No rug possible.\nLoad up before it runs.',
    'What if the next gem was right here?\n\n$FWC26 \u2014 zero dev, 100% community.\nRenounced. LP LOCKED. Low cap. Real conviction.',
    'Don\u2019t sleep on $FWC26.\nCommunity took over. Dev is gone. LP LOCKED.\nThis is what conviction looks like. Load up.',
  ];
...
bot.on('message',async function(ctx){
  ...
  if(admin&&!isPrivate){
    ...
    if(caW.some(function(w){return lower===w||lower.includes(w);})){
      ...
      await sendWithTracker(caMsg,ctx.chat.id,'$FWC26 Contract Address',{});return ctx.reply('<code>'+CA+'</code>',{parse_mode:'HTML'});
    }
    if(lower==='x'||lower==='twitter')return sendWithTracker(xMsg,ctx.chat.id,'Follow $FWC26 on X',{reply_markup:{inline_keyboard:[[{text:'Follow on X',url:TWITTER}]]}});
    if(lower==='socials'||lower==='links')return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xC4c6EaC95c5619F45714b74a4c4503f00af1FA26\'> Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xC4c6EaC95c5619F45714b74a4c4503f00af1FA26\'> PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});
    return;
  }
  if(!text)return;
  ...
  if(lower2==='x'||lower2==='twitter'||lower2.includes('follow on'))return sendWithTracker(xMsg,ctx.chat.id,'Follow $FWC26 on X',{reply_markup:{inline_keyboard:[[{text:'Follow on X',url:TWITTER}]]}});
  if(lower2==='socials'||lower2==='links')return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xC4c6EaC95c5619F45714b74a4c4503f00af1FA26\'> Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xC4c6EaC95c5619F45714b74a4c4503f00af1FA26\'> PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});
...
app.listen(PORT,async function(){console.log('$FWC26 bot port '+PORT);try{await new Promise(function(r){setTimeout(r,2000);});}catch(_){}try{await regWH();}catch(e){console.log(e.message);}if(parseInt(SIL_DELAY||'0')>0)try{resetSil();}catch(_){}try{schedShout();}catch(_){}setInterval(function(){if(WEBHOOK_URL)try{fetch(WEBHOOK_URL+'/health').catch(function(){});}catch(_){}},4*60*1000);console.log('$FWC26 bot live');});